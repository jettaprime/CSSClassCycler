(function(w, d) {
  ...
  ClassCycler.prototype._setStartIdx = function(config) {
    if (config && config.startIdx && config.startIdx.isWhole()) {
      this._cycleIdx = config.startIdx;
    } else {
      this._cycleIdx = 0;
    }
  };

  ClassCycler.prototype._setIntervalDuration = function(config) {
    var c = config;
    if (c && c.intervalDuration && c.intervalDuration.isWhole()) {
      this._intervalDuration = config.intervalDuration;
    } else {
      this._intervalDuration = ClassCycler.DefaultIntervalDuration;
    }
  };

  ClassCycler.prototype._setTimeoutDuration = function(config) {
    var c = config;
    if (c && c.timeoutDuration !== undefined) {
      this._timeoutDuration = config.timeoutDuration;
    } else {
      this._timeoutDuration = ClassCycler.DefaultTimeoutDuration;
    }
  };

  ClassCycler.prototype._setStepDistance = function(config) {
    var c = config;
    var direction = c && c.reverseDirection ? -1 : 1;
    var distance = c && c.stepDistance ? c.stepDistance : 1;
  ...
  ClassCycler.prototype._setAdditionalCallback = function(config) {
    var c = config;
    if (c && c.additionalCallback && isFunction(c.additionalCallback)) {
